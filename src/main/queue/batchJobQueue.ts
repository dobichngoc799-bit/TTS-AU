...
// Chạy tuần tự (concurrency = 1) để không dồn dập request vào GenVoice và dễ
// theo dõi rate limit (x-ratelimit-* — xem CLAUDE.md mục 2.1). Có thể tăng
// concurrency sau khi đo được rate limit thực tế.
export class BatchJobRunner {
  ...
  async run(): Promise<JobItem[]> {
    ...
    await fs.mkdir(this.job.outputDir, { recursive: true })
  ...
  private async processItem(item: JobItem): Promise<void> {
    try {
      ...
      const outputPath = join(this.job.outputDir, `${String(item.index + 1).padStart(3, '0')}.mp3`)
  ...
  private async joinAndMaybeWriteSrt(items: JobItem[]): Promise<void> {
    const done = items.filter((i) => i.status === 'done' && i.outputAudioPath)
    if (done.length === 0) return

    const joinedPath = join(this.job.outputDir, 'joined.mp3')
    await joinMp3Files(
    ...
    if (this.job.autoGenerateSrt) {
      ...
      await fs.writeFile(join(this.job.outputDir, 'joined.srt'), srt, 'utf-8')
    }
  }
